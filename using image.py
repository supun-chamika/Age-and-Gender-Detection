...
# subtraction to eliminate the effect of illumination changes
...
# download from: https://raw.githubusercontent.com/opencv/opencv_3rdparty/dnn_samples_face_detector_20180205_fp16
...
def predict_age(input_path: str):
    ...
    for i, (start_x, start_y, end_x, end_y) in enumerate(faces):
        ...
        for j in range(age_prediction[0].shape[0]):
            print(f"{AGE_INTERVALS[j]}: {age_prediction[0, j] * 100:.2f}%")
        j = age_prediction[0].argmax()
        age = AGE_INTERVALS[j]
        age_confidence_score = age_prediction[0][j]
        # Draw the box
